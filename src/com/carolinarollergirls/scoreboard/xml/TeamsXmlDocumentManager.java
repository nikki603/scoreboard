...
public class TeamsXmlDocumentManager extends DefaultXmlDocumentManager implements XmlDocumentManager
{
	...
	protected void processColor(Element newTeam, Element color) {
		...
		Element aColor = color.getChild("Color");
		if (null != aColor)
			editor.addElement(newColor, "Color", null, editor.getText(aColor));
	}
	...
	protected void toScoreBoard(String sbTeamId, String id, boolean reset) throws JDOMException {
		...
		while (colors.hasNext()) {
			...
			String aId = color.getAttributeValue("Id");
			if (null == aId || "".equals(aId.trim()))
				continue;
			String aColor = "";
			aColor = editor.getText(color.getChild("Color"));
			team.setColorModel(aId, aColor);
		}
