...
$sb(function() {
	...
	var showClocks = function() {
		...
		if (jR) {
			$("a.ClockJLT").closest("div").removeClass("ShowLineup ShowTimeout ShowFull").addClass("ShowJam");
			$("div.Ghostly").removeClass("ShowLineup ShowTimeout ShowFull").addClass("ShowJam");
		} else if (tR) {
			$("a.ClockJLT").closest("div").removeClass("ShowLineup ShowJam").addClass("ShowTimeout ShowFull");
			$("div.Ghostly").removeClass("ShowLineup ShowJam").addClass("ShowTimeout ShowFull");
		} else if (lR) {
			$("a.ClockJLT").closest("div").removeClass("ShowJam ShowTimeout").addClass("ShowLineup ShowFull");
			$("div.Ghostly").removeClass("ShowJam ShowTimeout").addClass("ShowLineup ShowFull");
		} else if (iR) {
			$("a.ClockJLT").closest("div").removeClass("ShowJam ShowTimeout ShowLineup ShowFull");
			$("div.Ghostly").removeClass("ShowJam ShowTimeout ShowLineup ShowFull");
		} else {
			$("a.ClockJLT").closest("div").removeClass("ShowLineup ShowTimeout ShowFull").addClass("ShowJam");
			$("div.Ghostly").removeClass("ShowLineup ShowTimeout ShowFull").addClass("ShowJam");
		}
	...
	$.each( [ "1", "2" ], function(i, team) {
		...
		$sb("ScoreBoard.Team("+team+").Color(overlay_fg).Color").$sbBindAndRun("sbchange", function(event,val) { 
			if (val == null || $.trim(val) == "") val = ''; 
			$("#Team"+team+"Name").css('color', val);
		});
		$sb("ScoreBoard.Team("+team+").Color(overlay_bg).Color").$sbBindAndRun("sbchange", function(event,val) { 
			if (val == null || $.trim(val) == "") val = ''; 
			$("#Team"+team+"Name").css('background-color', val);
		});
		...
		$.each( [ 0, 1, 2 ], function(x, i) {
			setupPulsate( 
				function() { 
					return ( $sb("ScoreBoard.Team(1).Timeouts").$sbGet() == i && 
			...
			setupPulsate( 
				function() { 
					return ( $sb("ScoreBoard.Team(2).Timeouts").$sbGet() == i && 
						$sb("ScoreBoard.Clock(Timeout).Running").$sbIsTrue() &&
						!$sb("ScoreBoard.OfficialReview").$sbIsTrue() && // Note, Negated. NOT Official Review
